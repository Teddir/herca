...
export const base_url = process.env.BASE_BE_URL
export const namespace = process.env.NAME_SPACE || "api/v1";
...
export const customError = (e: any) => {
  throw new Error(e?.error?.value?.summary || `${e?.error?.status}`)
};
